...
async function sendData(file, base64Content) {
  try {
    const response = await fetch(
      "https://t1oshk7xhi.execute-api.ap-south-1.amazonaws.com/Dev/upload",
      {
        method: "POST",
        headers: {
          "Content-Type": file.type, // Set the real MIME type
        },
        body: base64Content, // send raw base64 data
      }
    );
    ...
    if (result.S3Key) {
      ...
      await pollForResult(result.S3Key); // Use actual S3 key
    } else {
...
async function pollForResult(s3Key) {
  const filename = s3Key.split("/").pop().split(".")[0]; // extract filename without extension
  const cloudfrontURL = `https://d2vui8vf8m6hcm.cloudfront.net/ai-results/images/${filename}.json`; 
